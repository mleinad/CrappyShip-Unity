Shader "SEGI/SEGIVoxelizeScene_FAR_B" {
	Properties
	{
		...
		SubShader
		{
			...
			Pass
			{
				CGPROGRAM
					...
					float4x4 SEGIVoxelViewFront_FAR_B;
					float4x4 SEGIVoxelViewLeft_FAR_B;
					float4x4 SEGIVoxelViewTop_FAR_B;
					...
					void geom(triangle v2g input[3], inout TriangleStream<g2f> triStream)
					{
						...
						for (i = 0; i < 3; i++)
						{
							...
							if (angle == 0)
							{
								p[i].pos = mul(SEGIVoxelViewFront_FAR_B, p[i].pos);
							}
							else if (angle == 1)
							{
								p[i].pos = mul(SEGIVoxelViewLeft_FAR_B, p[i].pos);
							}
							else
							{
								p[i].pos = mul(SEGIVoxelViewTop_FAR_B, p[i].pos);
							}
